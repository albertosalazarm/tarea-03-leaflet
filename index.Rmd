...
## Acciones preliminares al mapa de coropletas
...
```{r message = FALSE}
# Full join de recursos
df_covid_cantones_actual <- 
 full_join(
    full_join(df_activos_cantones_ultima_fecha, df_fallecidos_cantones_ultima_fecha, by = c('cod_canton')), full_join(df_positivos_cantones_ultima_fecha, df_recuperados_cantones_ultima_fecha, by = c('cod_canton')))

# Objeto sf de casos covid agrupados por cantón en la última fecha
sf_covid_cantones <-
  left_join(cr_cantones, df_covid_cantones_actual, by = c('cod_canton')) 
...
## Mapa de coropletas
```{r message = FALSE}
bins1 <- c(0, 500, 1000, 10000, Inf)
paleta_rojo <- colorBin("Reds", domain = sf_covid$positivos, bins = bins1)

bins2 <- c(0, 100, 500, 1000, Inf)
paleta_naranja <- colorBin("Oranges", domain = sf_covid$activos, bins = bins2)

bins3 <- c(0, 50, 100, 200, Inf)
paleta_morado <- colorBin("Purples", domain = sf_covid$fallecidos, bins = bins3)

bins4 <- c(0, 1000, 5000, Inf)
paleta_azul <- colorBin("Blues", domain = sf_covid$recuperados, bins = bins4)
...
leaflet(sf_covid) %>% 
  ...
  addProviderTiles(providers$Stamen.TonerLite, group = "Stamen Toner Lite") %>%
   ...
   addPolygons(fillColor = ~paleta_rojo(positivos), stroke=T, fillOpacity = 1,
              color="black", weight=0.2, opacity= 0.5,
              group = "Positivos",
              popup = paste(
                        "Cantón: ", sf_covid_cantones$canton, "<br>",
                        "Positivos: ", sf_covid_cantones$positivos
                      ) ) %>%
   addLegend("topright", pal = paleta_rojo, values = ~positivos,
    ...
    addPolygons(fillColor = ~paleta_naranja(activos), stroke=T, fillOpacity = 1,
              ...
              popup = paste(
                        ...
                        "Cantón: ", sf_covid_cantones$canton, "<br>",
                        "Activos: ", sf_covid_cantones$activos
                      ) ) %>%
   addLegend("topright", pal = paleta_naranja, values = ~activos,
    title = "Casos activos",
    opacity = 1,
    group = "Activos"
  ) %>% 
    ...
    addPolygons(fillColor = ~paleta_morado(fallecidos), stroke=T, fillOpacity = 1,
              ...
              popup = paste(
                        ...
                        "Cantón: ", sf_covid_cantones$canton, "<br>",
                        "Fallecidos: ", sf_covid_cantones$fallecidos
                      ) ) %>%
   addLegend("topright", pal = paleta_morado, values = ~fallecidos,
    ...
    addPolygons(fillColor = ~paleta_azul(recuperados), stroke=T, fillOpacity = 1,
              ...
              popup = paste(
                        ...
                        "Cantón: ", sf_covid_cantones$canton, "<br>",
                        "Recuperados: ", sf_covid_cantones$recuperados
                      ) ) %>%
   addLegend("topright", pal = paleta_azul, values = ~recuperados,
